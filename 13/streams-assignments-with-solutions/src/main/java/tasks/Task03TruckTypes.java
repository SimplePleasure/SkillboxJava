...
public class Task03TruckTypes {
    ...
    public static TruckType getTypeByWeight(int weight) {
        int minFineWeight = Stream.of(TruckType.values()).flatMap(x -> Stream.of(x.maxLoad)).filter(x -> x >= weight).reduce((acc, x) -> acc>x?acc=x:acc)
                .orElseThrow(WeightTooHighException::new);
        return Stream.of(TruckType.values()).filter(x -> x.maxLoad == minFineWeight).findFirst().get();
    ...
    public static Map<TruckType, List<Truck>> groupTrucksByType(List<Truck> trucks) {
        ...
        return trucks.stream().collect(Collectors.groupingBy(x -> getTypeByWeight(x.maxWeightKg),
                TreeMap::new, Collectors.mapping(x-> x ,Collectors.toList())));
